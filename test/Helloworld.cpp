...
std::vector<std::string> ips {"172.31.8.98", "172.31.45.217", "172.31.41.196", "172.31.37.196", "172.31.36.162", "172.31.35.132"};
        ...
        for(u64 i = 0; i < nodePicker.generators.size(); i++) {
            ...
            for (u64 x: nodePicker.generators[i].second)
            {
                if(x != 0)
                {
                    ...
                    for (...;...;...;) 
                    { 
                        if ((clock() - initialTime) / CLOCKS_PER_SEC >= rebootTime)  break; 
                        u64 t = 4096;
                        u64 b = 128;
                        u64 a = 1024 / b;
                        cmd.setDefault("t", t);
